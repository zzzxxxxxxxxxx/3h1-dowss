...
export class WsDo extends DurableObject {
	 ...
	 * @param {{PYIP?:string, UUID4?:string, PASSWORD?:string, ENABLE_SS?:string|boolean, ALLOWED_RULES:string}} env
	 */
	constructor(state, env) {
		...
		this.allowedRules = (() => {
			...
			return raw.length > 0 ? raw : ["0.0.0.0/0", "::/0"];
		})();
	}
	...
	async fetch(request) {
		...
		const allowedRules = this.allowedRules;
...
export default {
	/**
	 * @param {Request} request
	 * @param {{PYIP?:string, UUID4?:string, PASSWORD?:string, ENABLE_SS?:string|boolean, ALLOWED_RULES:string, WS_DO: DurableObjectNamespace}} env
