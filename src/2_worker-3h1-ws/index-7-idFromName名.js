...
const ALLOWED_REGIONS = new Set(["wnam", "enam", "sam", "weur", "eeur", "apac", "oc", "afr", "me"]);
const urls = [
...
/**
 * 把 REGION 取出来、转成小写、校验合法性，合法则返回该值，否则返回默认值。
 * @param {object} obj   可能含有 REGION 的对象（例如 env 或 custom[index]）
 * @param {string} def   默认地区
 * @returns {string}
 */
function getValidRegion(obj, def) {
	const r = obj?.REGION?.trim().toLowerCase();
	return ALLOWED_REGIONS.has(r) ? r : def;
}

/**
 * 如果 OPT 存在且非空，返回 "-<trimmed>"；否则返回空字符串。
 * @param {string} opt
 * @returns {string}
 */
function makeOpt(opt) {
	return opt?.trim() ? `-${opt.trim()}` : "";
}

/**
 * 生成 Durable Object 的名字
 * @param {object} env  Cloudflare Workers 的 env 对象
 * @param {string} defaultRegion 默认地区，默认 "wnam"，可选值：["wnam", "enam", "sam", "weur", "eeur", "apac", "oc", "afr", "me"]
 * @returns {Array} 例如 ["user-<uuid>-<region>-<opt>", "wnam"]
 */
function makeDurableObjectName(env, defaultRegion = "wnam") {
	const uuid = env.UUID4?.trim() || "c3deb827-585e-4137-98a4-2dd4058f9836";

	// CUSTOM_VALUE & INDEX
	const custom = Array.isArray(env.CUSTOM_VALUE) ? env.CUSTOM_VALUE : [];
	const index = Number.parseInt(env.CUSTOM_INDEX, 10);

	// 先尝试用 CUSTOM_VALUE 里的配置
	if (custom.length && Number.isInteger(index) && index >= 0 && index < custom.length) {
		const cfg = custom[index];
		const region = getValidRegion(cfg, defaultRegion);
		const opt = makeOpt(cfg.OPT);
		return [`user-${uuid}-${region}${opt}`, region];
	}

	// 兜底
	const region = getValidRegion(env, defaultRegion);
	const opt = makeOpt(env.OPT);
	return [`user-${uuid}-${region}${opt}`, region];
}
...
export default {
	/**
	 * @param {Request} request
	 * @param {{PYIP?:string, UUID4?:string, PASSWORD?:string, ENABLE_SS?:string|boolean, REGION:string, OPT:string, CUSTOM_INDEX:string, CUSTOM_VALUE:Array, WS_DO: DurableObjectNamespace}} env
	...
	async fetch(request, env, ctx) {
		...
		const [name, region] = makeDurableObjectName(env, "wnam");
		const id = env.WS_DO.idFromName(name);
		// const id = env.WS_DO.newUniqueId();
		...
		const stub = env.WS_DO.get(id, { locationHint: region });
